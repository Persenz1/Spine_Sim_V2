"""Engagement angle and single-spine capacity helpers."""
...
class CapacityResult:
    """Single-spine tangential capacity result."""
...
def phi_s_deg_from_friction(f_s: float) -> float:
    """Return friction angle in degrees."""
    if f_s < 0.0:
...
def phi_hook_min_deg(alpha_p_deg: float, phi_s_deg: float) -> float:
    """Return the minimum hook angle from installation and friction angles."""
...
def compute_critical_angle_deg(
    *,
    ...
    """Compute phi_c after local preload W_i is known.

    For W_i <= 0, the spine is not eligible for engagement and NaN is returned.
    """
    if preload_n <= 0.0:
        return float("nan")
...
def effective_contact_angle_map_deg(
    ...
    *,
    ...
    """Compute a replaceable first-version effective contact angle map.

    The search direction is +x. With the surface normal convention from the
    model document, this reduces to atan(dz/dx) in degrees for the 2D search
    direction.
    """
...
def side_contact_risk_from_angle(
    *,
    ...
    """Return a conservative geometry risk marker for Phase 3 diagnostics."""
...
def compute_capacity_n(
    *,
    ...
    """Compute finite single-spine capacity without self-lock divergence."""
